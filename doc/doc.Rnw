...
number of its independence graph, which are undirected graphs with as many
...
Therefore the number of graphical models for a model with $k$ variables is $2^{k \choice 2}$.
...
of counts minus 1: $155519$ parameters.
